...
function devOnly_default({ interaction, targetCommand, handlerData }) {
  ...
  if (targetCommand.options?.devOnly) {
    if (interaction.inGuild() && !handlerData.devGuildIds.includes(interaction.guildId)) {
      interaction.reply({
        content: "\u274C This command can only be used inside SkyX Systems server.",
        ephemeral: true
      });
      return true;
    }
    ...
    if (!isDevUser) {
      interaction.reply({
        content: "\u274C This command can only be used by developers of SkyX Systems",
        ephemeral: true
      });
      return true;
    }
...
`;
  ...
  const embed = new import_discord.EmbedBuilder().setTitle(`:no_entry: Missing permissions!`).setDescription(embedDescription).setColor("Red");
