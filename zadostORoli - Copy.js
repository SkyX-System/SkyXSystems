  ...
  run: async ({ interaction, client }) => {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor('#ff0000') // Zvolte barvu embed
        .setTitle("Žádost o Roli")
        .addFields(
          { name: 'Jméno:', value: member.user.username, inline: true },
          { name: 'ID:', value: member.user.id, inline: true },
          { name: 'Datum:', value: new Date().toLocaleString(), inline: true },
          { name: 'Žádaná role:', value: roleName, inline: false } // Přidání názvu žádané role
        );
      ...
      const collector = interaction.channel.createMessageComponentCollector({ filter, max: 1, time: 60000 }); // Nastavte časový limit pro reakci (v milisekundách)
